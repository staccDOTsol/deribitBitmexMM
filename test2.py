...
while True:
	...
	lower = math.floor((spot - 500) / 1000) * 1000
	higher = math.ceil((spot + 500) / 1000) * 1000
	...
	for e in exps:
			...
			for s in strikes:
				...
				for o in options:
					if 'BTC' in options[o]['instrumentName'] and options[o]['instrumentName'] not in optionsignore:
						if options[o]['instrumentName'] in ivs.keys():
							iv = ivs[options[o]['instrumentName']]
							if iv != 0:
								exp2 = datetime.strptime(options[o]['expiration'][:-13], '%Y-%m-%d').strftime('%s')
								
								if((options[o]['optionType'] == 'call' and (options[o]['strike']) == lower) and exp2 == e):
									calls.append(s)
									#print(calls)
									civs[s] = iv
									pivs[s] = iv
								...
								if((options[o]['optionType'] == 'put' and (options[o]['strike']) == higher) and exp2 == e):
		...
		for c in calls:
			...
			for p in puts:
				pcount = pcount + 1
				c1 = black_scholes(spot, c, diff, pivs[p], 0.1, 0.0, 1) 
				p1 = black_scholes(spot, p, diff, civs[c], 0.1, 0.0, -1) 
			#	if c1 > costc[ccount] * spot:
			#		print(instsc[ccount])
			#		print(c1 / (costc[ccount] * spot))
		#		if p1 > costp[pcount] * spot:
	#				print(instsp[pcount])
#					print(p1/ (costp[pcount] * spot) )
				
				c2 = black_scholes(spot * 1.15, c, diff, pivs[p], 0.03, 0.0, 1) 
				p2 = black_scholes(spot * 1.15, p, diff, civs[c], 0.03, 0.0, -1) 
				c3 = black_scholes(spot * 0.85, c, diff, pivs[p], 0.03, 0.0, 1) 
				p3 = black_scholes(spot * 0.85, p, diff, civs[c], 0.03, 0.0, -1) 
				cost1 =(c1 + p1)
				cost2 = (c2 + p2)
				cost3 = (c3 + p3)
