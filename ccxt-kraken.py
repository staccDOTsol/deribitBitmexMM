...
class kraken(Exchange):
    ...
    def fetch_markets(self, params={}):
       ...
       # print(response)
        ...
        for i in range(0, len(keys)):
            id = keys[i]
            market = response['instruments'][id]
            baseId = market['symbol'][6:]
            quoteId = market['symbol'].split(baseId)[0][3:]
            #print('baseid: ' + baseId)
    ...
    def fetch_currencies(self, params={}):
        ...
        for i in range(0, len(ids)):
            ...
            code = self.safe_currency_code(self.safe_string(currency, 'pair'))
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'public':
            ...
            auth = self.encode(body + nonce + url)
            hash = self.hash(auth, 'sha256', 'binary')
            secret = base64.b64decode(self.secret)
            signature = self.hmac(hash, secret, hashlib.sha512, 'base64')
            headers = {
                'API-Key': self.apiKey,
                'API-Sign': (signature),
